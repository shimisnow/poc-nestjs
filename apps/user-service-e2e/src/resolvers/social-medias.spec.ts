...
describe('Social Medias Resolver', () => {
  let host: string;
  const endpoint = '/api';
  ...
  describe('authentication', () => {
    test('request without token', async () => {
      ...
      expect(result.data.socialmedia).toBeNull();
  ...
  describe('data request (role: user)', () => {
    test('exists', async () => {
      ...
      expect(result).toHaveProperty('data.socialmedia.identifier');
      expect(Object.keys(result.data.socialmedia).length).toBe(1);
      expect(result.data.socialmedia.identifier).toBe('@shimi_insta');
    });
    ...
    test('exists (nested query)', async () => {
      ...
      expect(result).toHaveProperty('data');
      expect(result).toHaveProperty('data.socialmedia.identifier');
      expect(result).toHaveProperty('data.socialmedia.user');
      expect(result).toHaveProperty('data.socialmedia.user.name');
      expect(result.data.socialmedia.identifier).toBe('@shimi_insta');
      expect(result.data.socialmedia.user.name).toBe('Shimi');
    });
